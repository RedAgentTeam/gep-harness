# ROADMAP v7.0 — GEP Harness Evolver 短暂回稳（exec 58/58 UNIQUE）
...
|--------|------|
| pytest (45/45) | ✅ 45/45 |
| `make verify` | ✅ 128/128（123 genes + 2 capsules + 3 events）|
...
**短暂回稳**：v6.9 反弹 → v7.0 回到 1 new 6 dup。

## v7.0 关键变更

| 项 | 详情 |
|----|------|
| 新增 Gene | 1 个 (exec_hotpath_v70) |
| 累计 Gene | 123 个 |
| DUPLICATE 处理 | 6 个 (read/process/write_file/edit/message/write 与 v1/v3/v5.0/v5.2/v6.0/v6.2/v6.5/v6.6/v6.7/v6.8/v6.9 同) |
| commit | 3ca012d |
| events | 10906 lines |
...
## 高频工具累计 UNIQUE 次数（v1.3~v7.0 共 58 期）
...
**Exec 58 期 58 UNIQUE 持续高频**——结构性问题必拆（4687 calls/24h）。
**Process 58 期 19 UNIQUE = 33%**——触发 UNIQUE 接近半数，强烈建议拆。
**Read 12 期 UNIQUE**（双位数）— 累计 8 次反弹。
**Message 13 期 UNIQUE**（双位数）— 累计 8 次反弹。
**Write 5 期 UNIQUE** — v1.5 之后 v6.1 反弹。

## 关键观察（v7.1 必拆 exec + process + message + read + write）

**v6.0 → v7.0 十一期趋势（双四连反弹周期 + 持续回稳 + 新反弹 + v6.3~v7.0 周期模式）**：
...
**v6.3~v7.0 持续回稳 6 期（v6.3/v6.4/v6.6/v6.7/v6.8/v7.0）+ v6.5/v6.9 反弹**——每 5-6 期出现一次反弹，周期稳定。

## 下一步建议

- [ ] **v7.1: exec args 前缀聚类专项**（58 期 58 UNIQUE，4687 calls/24h，必拆）
- [ ] **v7.1: process 超时防护场景细分**（19 期 UNIQUE，33% 占比，必拆）
- [ ] **v7.1: message 批量合并深挖**（13 期 UNIQUE，建议拆）
- [ ] **v7.1: read 模式聚类**（12 期 UNIQUE，可拆）
- [ ] **v7.1: write 模式聚类**（5 期 UNIQUE，可拆）
