# ROADMAP v6.3 — GEP Harness Evolver 短暂回稳（exec 51/51 UNIQUE）
...
|--------|------|
| pytest (45/45) | ✅ 45/45 |
| `make verify` | ✅ 119/119（114 genes + 2 capsules + 3 events）|
...
**短暂回稳**：v6.2 反弹延续 → v6.3 回到 1 new 6 dup。

## v6.3 关键变更

| 项 | 详情 |
|----|------|
| 新增 Gene | 1 个 (exec_hotpath_v63) |
| 累计 Gene | 114 个 |
| DUPLICATE 处理 | 6 个 (read/process/write_file/edit/message/write 与 v1/v3/v5.0/v5.2/v5.8/v6.0/v6.2 同) |
| commit | 9eb58c5 |
| events | 10412 lines |
...
## 高频工具累计 UNIQUE 次数（v1.3~v6.3 共 51 期）
...
**Exec 51 期 51 UNIQUE 持续高频**——结构性问题必拆（4445 calls/24h）。
**Process 51 期 18 UNIQUE = 35%**——触发 UNIQUE 接近半数，强烈建议拆。
**Read 12 期 UNIQUE**（双位数）— 累计 8 次反弹。
**Message 12 期 UNIQUE**（双位数）— 累计 7 次反弹。
**Write 5 期 UNIQUE** — v1.5 之后 v6.1 反弹。

## 关键观察（v6.4 必拆 exec + process + message + read + write）

**v5.0 → v6.3 十四期趋势（双四连反弹周期 + 持续回稳 + 新反弹 + v6.0~v6.3 周期模式）**：
...
每 3-4 期出现一次 1 new 6 dup 短暂回稳。

## 下一步建议

- [ ] **v6.4: exec args 前缀聚类专项**（51 期 51 UNIQUE，4445 calls/24h，必拆）
- [ ] **v6.4: process 超时防护场景细分**（18 期 UNIQUE，35% 占比，必拆）
- [ ] **v6.4: message 批量合并深挖**（12 期 UNIQUE，建议拆）
- [ ] **v6.4: read 模式聚类**（12 期 UNIQUE，可拆）
- [ ] **v6.4: write 模式聚类**（5 期 UNIQUE，可拆）
